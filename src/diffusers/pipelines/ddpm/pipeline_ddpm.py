...
class DDPMPipeline(DiffusionPipeline):
    ...
    @torch.no_grad()
    def clean_noisy(self, batch_size=1, generator=None, encoder_hidden_states=None, attention_mask=None, ground_truth_images=None, output_type="pil", **kwargs):
        if "torch_device" in kwargs:
            device = kwargs.pop("torch_device")
            warnings.warn("`torch_device` is deprecated. Use `pipe.to(torch_device)` instead.")
            # Set device as before (to be removed in 0.3.0)
            if device is None:
                device = "cuda" if torch.cuda.is_available() else "cpu"
            self.to(device)
